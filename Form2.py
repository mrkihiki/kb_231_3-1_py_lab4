...
class Form2(QMainWindow):
    def __init__(self, *args):
        ...
        self.ui.pushButton_2.clicked.connect(
            lambda: (
                ...
                setattr(self, 'bool', True)) if int(self.ui.spinBox.text()) in [item[0] for item in self.res] else None)
    ...
    def select_data(self):
        ...
        self.names = [item[0] for item in self.connection.cursor().execute("SELECT name FROM pragma_table_info('films')").fetchall()]
    ...
    def delete_elem(self):
        ...
        if id not in [item[0] for item in self.res]:
            return
